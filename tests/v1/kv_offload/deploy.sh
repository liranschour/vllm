...
# Decoder-only GPU memory utilization override. Defaults to GPU_MEM_UTIL.
# Lower it to shrink the decoder's KV-cache pool (e.g. to make cache capacity
# the bottleneck) without touching the prefiller.
DECODER_GPU_MEM_UTIL="${DECODER_GPU_MEM_UTIL:-$GPU_MEM_UTIL}"
...
# path (no kv_transfer_params on prefill; decode gets a top-level 'remote_kv_source' block).
P2P_MODE="${P2P_MODE:-false}"
# Proactive mode: exercise the proactive P2P migration control RPC. The proxy
# prefills, subscribes to the prefiller's KV-cache events to learn the stored
# block keys, triggers a migrate RPC on the decoder, then sends a plain decode
# request served from the prefetched CPU cache. Requires CONNECTOR=pd_connector.
PROACTIVE_MODE="${PROACTIVE_MODE:-false}"
# ZMQ PUB port for the prefiller's KV-cache events (proactive mode only).
...
# Proactive mode: enable KV-cache events on the prefiller so the proxy can learn
# the stored block keys. Byte-mode hashes (VLLM_KV_EVENTS_USE_INT_BLOCK_HASHES=0)
# are required so the proxy can rebuild exact OffloadKeys.
PREFILLER_KV_EVENTS_EXPORT=""
PREFILLER_KV_EVENTS_FLAG=""
if [[ "${PROACTIVE_MODE}" == "true" ]]; then
    PREFILLER_KV_EVENTS_EXPORT="export VLLM_KV_EVENTS_USE_INT_BLOCK_HASHES=0"
    _KV_EVENTS_JSON="{\"enable_kv_cache_events\":true,\"endpoint\":\"tcp://*:${KV_EVENTS_PORT}\",\"topic\":\"kv-events\"}"
    PREFILLER_KV_EVENTS_FLAG="--kv-events-config '${_KV_EVENTS_JSON}'"
fi
...
[[ "$CONNECTOR" == "pd_connector" && "$PROACTIVE_MODE" == "true" ]] && echo "  Proactive: on (--proactive; prefiller KV events port ${KV_EVENTS_PORT})"
echo "  Model:     ${MODEL}  gpu_mem=${GPU_MEM_UTIL}  max_len=${MAX_MODEL_LEN}"
[[ "${DECODER_GPU_MEM_UTIL}" != "${GPU_MEM_UTIL}" ]] && echo "  Decoder gpu_mem override: ${DECODER_GPU_MEM_UTIL} (shrunk KV cache)"
...
cleanup_pod() {
    ...
    # Kill the launcher AND the full VLLM:: process family. With
    # --data-parallel-size, vLLM spawns setproctitle-renamed children
    # (VLLM::APIServer_*, VLLM::DPCoordinator, VLLM::Worker) that match neither
    # "vllm serve" nor "VLLM::EngineCore"; SIGKILL on the parent orphans them
    # (reparented to PID 1), and the APIServer children keep the HTTP port bound
    # via SO_REUSEPORT — so a new deploy silently shares the port with the stale one.
    oc exec "${pod}" -- pkill -9 -f "vllm serve.*${http_port}" 2>/dev/null || true
    oc exec "${pod}" -- pkill -9 -f "VLLM::"                    2>/dev/null || true
    ...
    # SO_REUSEPORT lets a surviving API server share the HTTP port with the new
    # one instead of blocking its bind, so the collision is silent. Fail loudly
    # if the port is still held after the kills above (mirrors the proxy check).
    oc exec "${pod}" -- bash -c "
        echo -n 'Waiting for ${role} HTTP port ${http_port} to be free ...'
        deadline=\$(( \$(date +%s) + 30 ))
        while ss -tlnp 2>/dev/null | grep -q ':${http_port}[[:space:]]'; do
            if [[ \$(date +%s) -ge \$deadline ]]; then
...
# ---------------------------------------------------------------------------
if [[ "${PROACTIVE_MODE}" == "true" && "${CONNECTOR}" != "pd_connector" ]]; then
    echo "ERROR: PROACTIVE_MODE=true requires CONNECTOR=pd_connector (got '${CONNECTOR}')." >&2
    exit 1
fi

if [[ "${CONNECTOR}" == "pd_connector" ]]; then
    # host/port omitted from config: the P2P manager binds the
    # VLLM_P2P_SIDE_CHANNEL_HOST exported in each launcher (set to the pod IP
    # below, since the vLLM default is loopback-only 'localhost') and resolves
    # the port from VLLM_P2P_SIDE_CHANNEL_PORT (5710 default; decoder overrides
    # below).
    # Proactive mode enables self-describing KV events on the prefiller so the
    # proxy can reconstruct OffloadKeys from the published BlockStored events.
    _PREFILLER_EXTRA_KV=""
    [[ "${PROACTIVE_MODE}" == "true" ]] && _PREFILLER_EXTRA_KV=",\"self_describing_kv_events\":true"
    PREFILLER_KV_CONFIG="{\"kv_connector\":\"OffloadingConnector\",\"kv_role\":\"kv_both\",\"kv_connector_extra_config\":{\"spec_name\":\"TieringOffloadingSpec\",\"cpu_bytes_to_use\":${CPU_BYTES},\"secondary_tiers\":[{\"type\":\"p2p\"}]${_PREFILLER_EXTRA_KV}}}"
...
exec ${VLLM_BIN} serve '${MODEL}' \
    ...
    --enforce-eager \
    --block-size ${BLOCK_SIZE} \
    --gpu-memory-utilization ${DECODER_GPU_MEM_UTIL} \
    --max-model-len ${MAX_MODEL_LEN} \
...
if [[ "${CONNECTOR}" == "pd_connector" ]]; then
    ...
    # Proactive mode is exclusive with --p2p/--decoder-first: the proxy runs its
    # own prefill -> migrate -> decode flow and subscribes to the prefiller's
    # KV-cache events to learn the block keys to migrate.
    _PROACTIVE_FLAG=""
    if [[ "${PROACTIVE_MODE}" == "true" ]]; then
        _PROACTIVE_FLAG="--proactive --kv-events-endpoint tcp://${PREFILLER_ADDR}:${KV_EVENTS_PORT} --kv-events-topic kv-events"
        _P2P_FLAG=""
        _DECODER_FIRST_FLAG=""
    fi
